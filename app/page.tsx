...
export default function Home() {
  ...
  function continueDiagnostic() {
    ...
    if (!diagnosticFeedback) {
      ...
      const feedback = diagnosticStep === 0
        ? words.includes("skew") || words.includes("outlier") || words.includes("long")
          ? "Good. You connected the difference between mean and median to a pattern in the values. Before we move on, ask yourself: would a few very short stays create the same shape?"
          : "You made an initial claim — good. Now sharpen it: which summary is more affected by a handful of very long stays, the mean or the median?"
        : diagnosticStep === 1
          ? words.includes("context") || words.includes("clinical") || words.includes("error")
            ? "Exactly: unusual is not the same as wrong. Clinical context comes before any automated rule for removing values."
            : "Pause before changing the value. What clinical story might produce it, and what other fields could help you check that story?"
          : words.includes("categor") || words.includes("ordinal") || words.includes("continuous")
            ? "Nice classification thinking. The next step is to state why the nature of a value—not how it looks on a screen—determines its data type."
            : "You’ve made a start. Try naming the three possibilities: label, ordered category, or measurement. Then match each variable to one.";
