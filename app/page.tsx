...
export default function Home() {
  ...
  return (
    <main className="shell">
      ...
      {view === "lesson" && <section className="mc-panel lesson-mc" aria-label="Data type multiple choice"><p className="eyebrow"><span></span> DATA TYPES · CHECK YOUR UNDERSTANDING</p><h2>What kind of variable is discharge disposition?</h2><p className="mc-question">The values are “home,” “skilled nursing facility,” and “hospice.” Choose the most accurate classification.</p><div className="mc-choices">{["Continuous, because the options describe a range of patient outcomes.", "Ordinal, because each discharge location has a fixed clinical ranking.", "Categorical, because the values are labels rather than measurements.", "Binary, because there are only two types of patients."].map((choice, index) => <button key={choice} className={`mc-choice ${lessonChoice === index ? "selected" : ""} ${lessonChecked && index === 2 ? "correct" : ""} ${lessonChecked && lessonChoice === index && index !== 2 ? "incorrect" : ""}`} disabled={lessonChecked} onClick={() => setLessonChoice(index)}><span>{String.fromCharCode(65 + index)}</span>{choice}</button>)}</div>{lessonChecked && <div className={`mc-result ${lessonChoice === 2 ? "right" : "wrong"}`}><b>{lessonChoice === 2 ? "Correct" : "Not quite"}</b><p>{lessonChoice === 2 ? "Correct. The values are labels for groups, so discharge disposition is categorical. Their clinical meaning does not create a universal numerical scale." : "The best answer is C. Discharge locations are categories; there is no universally fixed numerical order from home to hospice."}</p></div>}<div className="mc-actions"><button className="hint" onClick={() => setLessonHint(true)}>Need a hint?</button><button className="primary" disabled={lessonChoice === null} onClick={() => lessonChecked ? setView("overview") : setLessonChecked(true)}>{lessonChecked ? "Continue learning" : "Check answer"}<b>→</b></button></div>{lessonHint && <div className="hint-card"><span>NUDGE</span><p>Ask whether these values measure an amount or simply name a group.</p></div>}</section>}
